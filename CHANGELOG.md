...
- **`crash_window` is now default-ON** (`src/daemon/agent-process.ts`). The sliding-window CrashLoopPauser has existed since the instar port but was opt-in, and no shipped config set it — so `crashWindowMs` was 0 for every real agent and the pauser never ran. The only backstop was `max_crashes_per_day` (10) combined with a crash backoff that caps at 5 minutes, which respawned a broken agent roughly every 5 minutes for the better part of an hour, every day, overnight included. New defaults: **3 crashes in 30 minutes → `halted`** (`DEFAULT_CRASH_WINDOW_SECONDS` / `DEFAULT_CRASH_WINDOW_MAX_CRASHES`, both exported). A halt already raises the existing Telegram alert, so the pause is visible rather than silent. Rate-limit exits and image-poison recoveries return before the window check and are still not counted.
  - **Opt out** with an explicit `crash_window: { seconds: 0 }` to restore daily-counter-only behaviour. An explicit `crash_window` continues to win over the default.
  - Applies to existing agents on next daemon start — no config edit required.
- **Shipped agent configs now declare a `shift_schedule`** (weekdays 08:00–22:00, weekends 08:00–21:00, agent timezone). `shift_schedule` is the daemon's only real quiet-hours gate — `day_mode_start` / `day_mode_end` are prose-only, interpolated into IDENTITY.md and printed by `bus get-config` but read by no daemon code path. Because no shipped config set `shift_schedule`, `evaluateShift()` received `undefined` and returned `in_shift` unconditionally, so the 4-hourly "work on your highest priority task" heartbeat fired at 02:00 like any other hour.
  - **Inbound is unaffected.** The gate only suppresses daemon-scheduled cron fires. Telegram, Slack, and Gmail inbound are not shift-gated, so an off-hours message still reaches the agent immediately.
  - **`wake_on_fire: true` added to unattended maintenance crons** (`nightly-metrics`, `nightly-review`, `auto-commit`, `check-upstream`, `catalog-browse`, `followup-sweep`). Off-hours execution is their purpose, and a suppressed fire re-schedules to the same wall-clock slot a full period later — so a ≥24h cron that lands off-shift once would be suppressed at that time every day thereafter, silently.
  - Hermes agents are excluded: `startAgentCronScheduler()` returns early for them, so a `shift_schedule` there would be inert config that reads as protection it does not provide.
  - Existing agents are unaffected until their `config.json` gains a `shift_schedule`; run `cortextos bus reload-crons <agent>` afterwards to propagate `wake_on_fire`.
- **`cortextos bus set-shift` CLI** (`src/bus/shift-config.ts`, `src/cli/bus.ts`): applies quiet hours to an existing fleet, since templates only seed newly-created agents. `set-shift <agent>` or `--all`, with `--weekday` / `--weekend` (`HH:MM-HH:MM`, `off`, or `24h`), `--clear` to restore 24/7, plus `--dry-run`, `--json`, and `--force`. Writes atomically with a `.bak`, is idempotent, and skips Hermes agents.
  - **Refuses by default when a window would strand a cron.** Suppression is silent-by-design in the daemon — a suppressed fire is recorded as a success and `nextFireAt` advances a full period — so a daily cron outside the window is suppressed at that time every day thereafter with no error anywhere. `set-shift` evaluates every agent dry first and aborts the whole run before writing anything, so a fleet edit cannot land half-applied. Long-period interval crons and unparseable cron expressions are reported as advisories rather than blocking.
- **Known gap (not addressed here)**: `CronEntry.emergency_allowed` is declared in `src/types/index.ts` but implemented nowhere, so `off_shift_emergency_only` and `off_shift_no_wake` suppress identically today — the `emergency_override` allowlist only changes the telemetry label. The shipped schedules therefore use plain windows rather than implying an emergency path that does not exist.
- **Regression tests**: `tests/integration/shipped-configs-quiet-hours.test.ts` asserts every cron-bearing shipped config declares a shift schedule, that no time-anchored cron falls outside its own window (the window end is exclusive, so an 18:00 cron needs a window ending strictly after 18:00), and that stranding-risk interval crons are flagged `wake_on_fire`.
